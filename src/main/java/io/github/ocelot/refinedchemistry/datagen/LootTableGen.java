...
public class LootTableGen extends ForgeLootTableProvider
{
    ...
    private static class EntityProvider extends EntityLootTables
    {
        ...
        {
            return ForgeRegistries.ENTITIES.getValues().stream().filter(entityType -> entityType.getRegistryName() != null && RefinedChemistry.MOD_ID.equals(entityType.getRegistryName().getNamespace())).collect(Collectors.toSet());
        }
    }
    ...
    private static class BlockProvider extends BlockLootTables
    {
        ...
        {
            return ForgeRegistries.BLOCKS.getValues().stream().filter(entityType -> entityType.getRegistryName() != null && RefinedChemistry.MOD_ID.equals(entityType.getRegistryName().getNamespace())).collect(Collectors.toSet());
        }
    }
